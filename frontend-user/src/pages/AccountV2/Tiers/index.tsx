...
const Tiers = (props: any) => {
  ...
  return (
    ...
    >
      <div className={styles.titleComponent}>My Tier</div>
      <div className={styles.desTitleComponent1}>You are in Tier Dove You are in Tier Dove 🕊</div>
      <div className={styles.desTitleComponent2}>Please stake tokens in your wallet balance to upgrade your tier!</div>
          ...
          <>
            {verifiedEmail && connectedAccount ? (
              <p>
                {userTier > 0
                  ? `You are in tier ${
                      getUserTierAlias(userTier as number).text
                    }`
                  : "You are not in any tier yet"}
                . To upgrade your tier, please click{" "}
                <Link to="/account" className={styles.tierLinkToAccount}>
                  here
                </Link>
                .
              </p>
            ) : (
              <p>You are not in any tier yet.</p>
            )}
          </>
        </div>
      )} */}
      ...
      <ul className={styles.tierList}>
        <li className={`${styles.tierInfo} active first-tier`}>
          {isWidthUp("sm", props.width) && connectedAccount && (
            <span
              ...
              style={{
                ...
                width:
                  userTier > 0 ? "calc(100%)" : `${currentProcess || 0}%`,
              }}
            ></span>
          )}
          {isWidthDown("xs", props.width) && connectedAccount && (
            <span
          ...
          <div>
            ...
            <div className="info">
              <span className={`active ${styles.tierName}`}>{TIERS[0].name}</span>
              <span className={styles.tierValue}>{tokenSymbol}</span>
            </div>
          </div>
        </li>
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <div
                      className="icon"
                      style={hideStatistics && { marginTop: -30 }}
                    >
                      {
                        userTier <= idx ?
                        <>
                          <img className={styles.iconCheck} src="/images/icons/icon_not_tick.svg" alt="" />
                          <img className={styles.bgIconCheck} src="/images/icons/tiers_not_active.svg" alt="" />
                        </>
                        :
                        <>
                          <img className={styles.iconCheck}  src="/images/icons/icon_tick.svg" alt="" />
                          <img className={styles.bgIconCheck} src={TIERS[idx + 1].bg} alt="" />
                        </>
                      }
                    ...
                    <div className="info">
                      <span className={`${userTier > idx ? "active " : " "} ${styles.tierName}`}>{TIERS[idx + 1].name}</span>
                      {!showMoreInfomation && (
                        <span className={styles.tierValue}>
                          {numberWithCommas(tier)} {tokenSymbol}
                        </span>
                      )}
                      {showMoreInfomation && !hideStatistics && (
                        <span className={styles.tierValue}>
                          {numberWithCommas(tiersBuyLimit[idx + 1])}{" "}
                          {tokenSymbol}
                        </span>
                      )}
                    </div>
      ...
      {!showMoreInfomation && (
        <div className={styles.tierNote}>
          <h3 className={styles.title}>
            ...
            <Tooltip
              placement="top-start"
              classes={{ tooltip: commonStyle.tooltip }}
              enterDelay={500}
              leaveDelay={200}
              title={
                <p style={{ font: "normal normal normal 12px/18px Helvetica" }}>
                  Equivalent PKF = PKF
                  {rates.data &&
          ...
          <span className={styles.subtitle}>
            {connectedAccount && isAuth && !wrongChain
          ...
          {/* <div className="notice">
            <img src={TIERS[userTier].icon} alt=""/>
            <div className="notice-content">
              {userTier > 0 && connectedAccount ? (
                <span>You are in Tier {TIERS[userTier].name}</span>
              ) : (
                <span>You are not in any tier yet.</span>
              )}
              <span>
                Please stake tokens in your wallet balance to maintain your
                tier!
              </span>
            </div>
          </div> */}
