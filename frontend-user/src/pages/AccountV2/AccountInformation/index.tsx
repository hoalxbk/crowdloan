...
const AccountInformation = (props: any) => {
  ...
  const handleKYC = () => {
    window.open(
      "https://verify-with.blockpass.org/?clientId=red_kite_kyc_7a0e6&serviceName=Red%20Kite%20KYC&env=prod",
      "_blank"
    );
  };
  ...
  return (
    <div className={styles.accountInfomationComponent}>
      <h2 className={styles.title}>Account</h2>
      <div className={styles.mainInfomation}>

        <div className={styles.inputGroup}>
          <span className={styles.nameInputGroup}>Your Wallet Address</span>
          <span className={styles.valueInputGroup}>
            {isWidthUp("sm", props.width) && connectedAccount}
            {isWidthDown("xs", props.width) &&
              connectedAccount &&
              trimMiddlePartAddress(connectedAccount || "")}
          </span>
        </div>

        <div className={styles.inputGroup}>
          <span className={styles.nameInputGroup}>KYC Status</span>
          {connectedAccount && (
            <>
              {kycStatus === KYC_STATUS.INCOMPLETE && (
                <span className={styles.valueInputGroup}>Unverified</span>
              )}
              {kycStatus === KYC_STATUS.INCOMPLETE && (
                <button className={styles.btnInputGroup} onClick={handleKYC}>
                  KYC NOW
                </button>
              )}

              {kycStatus === KYC_STATUS.APPROVED && (
                <span className={styles.valueInputGroup}>Verified</span>
              )}

              {kycStatus === KYC_STATUS.RESUBMIT && (
                <span className={styles.valueInputGroup}>Rejected</span>
              )}
        ...
        <div className={styles.inputGroup}>
          <span className={styles.nameInputGroup}>Email</span>
          <span className={styles.valueInputGroup}>
            Your email used to complete KYC on Blockpass will be automatically
            reflected here.
            <br /> Please complete KYC first.
          </span>
          {/* {email && emailVerified !== USER_STATUS.UNVERIFIED && (
            <span>{email}</span>
          )}
        ...
        {/* <div className={styles.inputGroup}>
          <span className={styles.nameInputGroup}>Your Tier</span>
          <span>
