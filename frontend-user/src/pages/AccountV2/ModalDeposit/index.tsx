...
const ModalDeposit = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const tokenBalance = new BigNumber(currentBalance).multipliedBy(
        new BigNumber(10).pow(18)
      );
      const amount = new BigNumber(depositAmount).multipliedBy(
        new BigNumber(10).pow(18)
      );
  ...
  useEffect(() => {
    if (listTokenDetails.length == 0 || rates.length == 0 || !currentToken)
      return;
  ...
  const onDeposit = () => {
    ...
    dispatch(
      deposit(connectedAccount, depositAmount, library, currentToken.address)
    );
  ...
  const handleSelectToken = (e: any) => {
    ...
    if (e.target.value == "PKF") {
      ...
      (e.target.value == CONVERSION_RATE[0].key &&
        appChainID.appChainID == "5") ||
      (e.target.value == CONVERSION_RATE[0].keyMainnet &&
        appChainID.appChainID == "1")
    ) {
      setCurrentBalance(balance.uni);
      setCurrentStaked(userInfo.uniStaked);
      setCurrentAllowance(allowance.uni);
    } else if (
      (e.target.value == CONVERSION_RATE[1].key &&
        appChainID.appChainID == "5") ||
      (e.target.value == CONVERSION_RATE[1].keyMainnet &&
        appChainID.appChainID == "1")
    ) {
    ...
    >
      <div className="modal-content">
        ...
        >
          ...
          />
          <h2 className="title">
            You have {numberWithCommas(totalStaked)}{" "}
            {listTokenDetails[0]?.symbol} staked
          </h2>
          <p className={styles.description}>
            {currentAllowance == "0"
              ? "You need to Approve once (and only once) before you can start staking."
        ...
        <DialogContent className="modal-content__body">
          {open && (
            ...
            >
              {listTokenDetails?.map((tokenDetails: any, index: number) => {
                  return (
                    <option value={tokenDetails?.symbol} key={index}>
                      {index === 0
                        ? "Polkafoundry (PKF)"
                        : `${CONVERSION_RATE[index - 1].name} (${
                            CONVERSION_RATE[index - 1].symbol
                          })`}
                    </option>
                  );
                })}
          ...
          <div className={styles.group}>
            <div className="balance">
              <div>
                ...
                <span>
                  {!currentBalance ? 0 : numberWithCommas(currentBalance)}{" "}
                  {currentToken?.symbol == "PKF"
                    ? "PKF"
                    : (appChainID.appChainID == "5" &&
                        currentToken?.symbol == CONVERSION_RATE[0].key) ||
                      (appChainID.appChainID == "1" &&
                        currentToken?.symbol == CONVERSION_RATE[0].keyMainnet)
                    ? CONVERSION_RATE[0]?.symbol
                    : CONVERSION_RATE[1]?.symbol}
                </span>
              </div>
            </div>
            ...
            <div className="balance" style={{ marginTop: "10px" }}>
              <div>
                ...
                <span>
                  {numberWithCommas(
                    (parseFloat(depositAmount) * currentRate || 0).toString()
                  )}{" "}
                  {listTokenDetails[0]?.symbol}
                </span>
              </div>
            </div>
          </div>
