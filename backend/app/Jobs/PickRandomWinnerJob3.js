...
class PickRandomWinnerJob3 {
  ...
  static async handle(data) {
    console.log('PickRandomWinnerJob3-job started', data);
    try {
      // do snapshot balance
      await PickRandomWinnerJob3.doSnapshotBalance(data);
      // pickup random winner after snapshot all whitelist user balance
      await PickRandomWinnerJob3.doPickupRandomWinner(data);
    } catch (e) {
  ...
  static async doPickupRandomWinner(data) {
    ...
    for (let i = 0; i < data.tiers.length; i++) {
      ...
      if (tier.level == 1) {
        ...
        winners = await PickRandomWinnerJob3.pickupRandom(userSnapshots.toJSON(), tier.ticket_allow, data.campaign_id);
      } else {
...
module.exports = PickRandomWinnerJob3
